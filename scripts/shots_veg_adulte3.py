#!/usr/bin/env python3
"""Captures ADULTES post-fix v2 : planterParId puis surcharge maturite par
instance (0.55-1.0) + reappliquerEtat. Vue dessus/dessous, 3 saisons + info."""
...
# 50 instances via l'API existante (maturite 0.12 par defaut)
JS_PLANTER = '''(() => {
  ...
  // Surcharge maturite : jardin etabli (adultes)
  for (const inst of J.jardin.instances) {
    inst.maturite = 0.55 + Math.random() * 0.45;
  }
...
time.sleep(14)
print('scales:', eval_js('''JSON.stringify(window.__jardin.jardin.instances.slice(0,5).map(i => ({id: i.plante.id, mat: +i.maturite.toFixed(2), scale: +i.group.scale.x.toFixed(2), veg: i.vegActif})))'''))
