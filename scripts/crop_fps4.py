"""Analyse des captures FPS par contrôleur (jour/automne/hiver)."""
from PIL import Image
import os

BASE = os.path.join(os.path.dirname(__file__), '..', 'preuves')
for name in ['veg_fps_jour.png', 'veg_fps_automne.png', 'veg_fps_hiver.png']:
    img = Image.open(os.path.join(BASE, name)).convert('RGB')
    w, h = img.size
    centre = img.crop((int(w*0.25), int(h*0.15), int(w*0.75), int(h*0.75)))
    out = name.replace('.png', '_crop.png')
    centre.save(os.path.join(BASE, out))
    px = list(centre.getdata())
    n = len(px)
    moy = tuple(round(sum(c[i] for c in px)/n) for i in range(3))
    verts = sum(1 for r, g, b in px if g > r+15 and g > b+15)
    oranges = sum(1 for r, g, b in px if r > g+15 and g > b+10)
    bruns = sum(1 for r, g, b in px if r > 90 and g > 60 and b < 80 and abs(r-g) < 40)
    print(out, 'moyRGB=', moy, f'verts={100*verts//n}% oranges={100*oranges//n}% bruns={100*bruns//n}%')
